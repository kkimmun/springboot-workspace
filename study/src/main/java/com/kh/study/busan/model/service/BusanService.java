...
public class BusanService {
	...
	public String getRes(int page) {
		...
		String url ="https://apis.data.go.kr/6260000/FoodService/getFoodKr?serviceKey=83aa005b8121e0f328d2ab6a340461e452158b3899f5c1c8e824895d00a781e8&numOfRows=6&resultType=json";
		url += "&pageNo=" + page;
...
String apiResponse = new RestTemplate().getForObject(uri, String.class);
	...
	public void save(Long ucSeq, ReviewDto review) {
		...
		if(review.getRating() < 1 || 5 < review.getRating()) {
			...
			// 그럼 비즈니스 로직은 어디에 넣지? => VO => 초고수면 그렇게 가는거고
			// 
		}
		mapper.save(review);
	}
