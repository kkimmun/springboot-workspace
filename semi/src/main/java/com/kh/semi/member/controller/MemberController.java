...
public class MemberController {
	 ...
	 * -> 일치하면 사용자 정보를 HttpSession(서버 메모리)저장
	 *  
	 * 서버는 응답 헤더의 Set-Cookie: JSESSIONID=어쩌고저쩌고~~를 클라이언트에게 응답
	 * 다음 요청부터 클라이언트는 같은 도메인이라면 Cookie:JSESSIONID=어쩌고저쩌고~~를 같이 보내서
	 * 
	 ...
	 * 
	 * => 토큰 기반 인증방식 도입
	 * => 서버가 클라이언트의 정보를 계속 들고있다 => 인증된 사용자의 정보를 서버에서 보관하지 말자! => 모든 요청이 자기 스스로를 증명하게 만들자! => 토큰 기반 인증방식
	 * 
	 * => JWT가 표준 구현
	 * 
	 * 1. 표준 사양
